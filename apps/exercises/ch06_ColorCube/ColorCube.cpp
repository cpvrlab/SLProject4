/**
 * \file      ColorCube.cpp
 * \details   Core profile OpenGL application with a colored cube with
...
static GLint cLoc;  //!< attribute location for vertex color
...
void buildBox()
{
    ...
    numV = 8;
    std::vector<VertexPC> vertices(numV);
...
void onInit()
{
    ...
    shaderVertID = glUtils::buildShader(projectRoot + "/data/shaders/ch06_ColorCube.vert", GL_VERTEX_SHADER);
    shaderFragID = glUtils::buildShader(projectRoot + "/data/shaders/ch06_ColorCube.frag", GL_FRAGMENT_SHADER);
    ...
    cLoc  = glGetAttribLocation(shaderProgID, "a_color");
...
bool onPaint()
{
    ...
    glDrawElements(GL_TRIANGLES, (GLint)numI, GL_UNSIGNED_INT, nullptr);
...
int main(int argc, char* argv[])
{
    ...
    initGLFW(scrWidth, scrHeight, "ColorCube");
